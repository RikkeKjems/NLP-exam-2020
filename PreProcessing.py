...
path = glob.glob(
    "Data/Lemma_data/ND_lemma/ND21_lemma.txt"
...
for word in words:
    ...
    colm = ["Freq"]
    df = pd.DataFrame(
        data=values, index=keys, columns=colm
    )  # df with filename and frequency of ech word in file
...
procent = (num / len(df_ND)) * 100  # changes numbers for each file in each folder
...
ND21_21 = df5.rename(
    index={0: "ND21"},
    columns={"variable": "Most common word Length", "value": "Occurence"},
)
ND21_21
...
# Creating new df with percentage values
df_procent = pd.DataFrame(data=procent, index=["ND21"], columns=["Unique occurence %"])
