...
new_raw_data = df.drop(columns=['File'])
...
for i, column in enumerate(raw_data.drop('D_or_ND', axis=1)):
...
df = df.drop(columns=['Unnamed: 0','File', 'Occurence', 'Unique_words_in_doc', 'no_words', 'no_useful_tokens', 'no_useless_tokens'])
...
df = df[df['max_word_length'] < 60]

df.shape

# %%
# Visualize the data using seaborn Pairplots
g = sns.pairplot(df, hue='D_or_ND')
...
features = ['D_or_ND', 'max_word_length', 'Most_common_word_length',
       'Occurence_perc', 'Noun_perc', 'Verb_perc', 'Adj_perc', 'Pron_perc',
       'Adv_perc', 'Prop_perc']

#%%
## PLOT MED MANGE PLOTS
for f in features:
    sns.countplot(x=f, data=df, palette='Set3', hue='D_or_ND')
    plt.show()
...
# Creating columns to keep
columns_to_keep = fi_df['Feature'][0:7]
